...
if ($auth->userOn()) {
  ...
  $group_data = $user->getGroupsData(); 

  $smarty->assign("group_data",  $group_data);
  $start = getmicrotime();
  $content .= $smarty->fetch('smarty_tpl/user_management_panel_groups.smarty');    
  $stop = getmicrotime();
  if (isset($_GET['debugphp']))
    echo 'smarty_tpl/user_management_panel_groups.smarty'.($stop-$start);
