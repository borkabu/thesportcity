...
<?php 
   if ($_SERVER["HTTP_HOST"] == "www.eurofootballlt.thesportcity.net" || $_SERVER["HTTP_HOST"] == "eurofootballlt.thesportcity.net") {
     $style= "eurofootball.lt";
     if (!isset($_SESSION['external_user']['HOST'])) {
       if ($_SERVER["HTTP_HOST"] == "www.eurofootballlt.thesportcity.net")
         $_SESSION['external_user']['HOST'] = "www.eurofootball.lt";
       else if ($_SERVER["HTTP_HOST"] == "eurofootballlt.thesportcity.net")
         $_SESSION['external_user']['HOST'] = "eurofootball.lt";
     }
   } else if ($_SERVER["HTTP_HOST"] == "www.ivartisnet.thesportcity.net" || $_SERVER["HTTP_HOST"] == "ivartisnet.thesportcity.net") {
     $style= "ivartis.net";
   } else {
     $style= "eurofootball.lt";
   }
...
?>
<link rel="stylesheet" href="lib/style_<?php echo $style?>.css?sdfsfsfdsdsdswssdsde" type="text/css" />
...
<?php 
 ...
 if ($_SERVER["HTTP_HOST"] == "www.eurofootballlt.thesportcity.net" || $_SERVER["HTTP_HOST"] == "eurofootballlt.thesportcity.net") {
  include('inc/top_external_add_eurofootball.lt.inc.php');
 } else if (isset($clients[$_SESSION['external_user']['SOURCE']]['adds_external']) 
     && $clients[$_SESSION['external_user']['SOURCE']]['adds_external'] == true) {
  include('inc/top_external_add_'.$_SESSION['external_user']['SOURCE'].'.inc.php');
 } else {
?>
...
<script type="text/javascript">
  function iframeResizePipe()
  {
     ...
     <?php 
       if (!isset($_SESSION['external_user']['SOURCE'])) {
     ?>
     refresh = "&refresh=true";
     <?php } ?>
     pipe.src = '<?php echo "http://".$_SESSION['external_user']['HOST'];?>/helper.php?height='+height+'&cacheb='+Math.random()+refresh;
...
<br>
<?php echo $external_menu->getMenu(scriptName($_SERVER["PHP_SELF"]), "left", 150, 2, 0, 20) ?>
</div>
</div>
...
<?php 
 if ($_SERVER["HTTP_HOST"] == "www.eurofootballlt.thesportcity.net" || $_SERVER["HTTP_HOST"] == "eurofootballlt.thesportcity.net") {
   ...
   echo get_0x0();
            ...
            && $clients[$_SESSION['external_user']['SOURCE']]['adds_external'] == false
            && isset($clients[$_SESSION['external_user']['SOURCE']]['adds']['468x60'])) { ?>
  <iframe id="tsc_add_468x60" src="<?php echo $clients[$_SESSION['external_user']['SOURCE']]['adds']['468x60']?>" width="468px" height="60px" frameborder="no" style="border-width:0px;overflow-x:hidden">
