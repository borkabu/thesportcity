...
// include common header
  ...
  if ($auth->userOn()) {
   if (isset($_GET['action']) && $_GET['action']=='accept_invite' && isset($_GET['league_id'])) {
     ...
     if ($row = $db->nextRow()) {
       ...
       if ($row['ENTRY_FEE'] == 0) {
         ...
         $db->update('solo_manager_leagues_members', $udata, 'USER_ID='.$auth->getUserId().' AND LEAGUE_ID='.$_GET['league_id']);
         unset($udata);
   ...
   if (isset($_GET['action']) && $_GET['action']=='decline_invite' && isset($_GET['league_id'])) {
     $db->update('solo_manager_leagues_members', "STATUS=5", 'USER_ID='.$auth->getUserId().' AND STATUS=3 AND LEAGUE_ID='.$_GET['league_id']);
   }
  } 
  ...
  $content = $managerbox->getSoloManagerLeagueBox($manager_user);
