...
if ($auth->userOn()) {
  ...
  if(isset($_POST['news_id']) && is_numeric($_POST['news_id'])) {
	...
	$reality = '';
	$data['LOGGED'][0]['REALITY'] = $frm->getInput(FORM_INPUT_SELECT, 'reality', $reality, $opt, $reality);
	$data['LOGGED'][0]['CAT_ID'] = inputCats('cat_id', isset($PRESET_VARS['cat_id']) ? $PRESET_VARS['cat_id'] : '', 80, true);
        $data['LOGGED'][0]['SUGGEST'][0]['X'] = 0;
        break;
     case 1:
	 // flood protection
     case 2:
...
if(isset($_POST['post_news']) && $auth->userOn() &&
   ((isset($_POST['news_id']) && is_numeric($_POST['news_id'])) || 
    empty($_POST['news_id']))){
      ...
      $news_id = $newsbox->addNewsItem($_POST['news_id'], 1);
      if ($news_id == '') {
...
if (!$added) {
  ...
  $tpl->setTemplateFile('tpl/news_add.tpl.html');
  $tpl->addData($data);
  $content = $tpl->parse();
}
