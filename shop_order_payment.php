...
if ($auth->userOn()) {
  if (isset($_POST['accept_payment'])) {
    $order_item = $shopbox->getShopOrderItemData($_POST['order_id']);    
    if ($order_item['TOTAL'] + $order['DELIVERY_CREDITS'] <= $_SESSION["_user"]['CREDIT']) {
      if ($shop->acceptPayment($_POST['order_id'])) {
        $errorbox = new ErrorBox($langs, $_SESSION["_lang"]);
        $errorbox1 = $errorbox->getMessageBox('MESSAGE_SHOP_ORDER_CONFIRMED');

        $order_item = $shopbox->getShopOrderItem($_POST['order_id']);
        if ($order_item != '') { 
          $content .= $order_item;
        }
      }
   ...
   $content .= $basketbox->getBasketCheckoutCredit();
  }
} else {
  $errorbox = new ErrorBox($langs, $_SESSION["_lang"]);
