...
class ManagerUserLog {
 
  function ManagerUserLog() {

  }

  function logEvent ($user_id, $event_type, $value, $season_id, $player_id='', $user2_id='', $value2=''){
    global $db;
    ...
    $db->insert('manager_users_log', $sdata);
  }


 function getManagerUserLog($user_id, $season_id, $page, $perpage) {
    global $manager_users_log_events_descr_success;
    global $db;
    global $auth;
...
//echo $sql;
    ...
    while ($row = $db->nextRow()) {
       ...
       $log_item['LOG_ENTRY'] = $manager_users_log_events_descr_success[$row['EVENT_TYPE']];
