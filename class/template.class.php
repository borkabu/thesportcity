...
//    $data['SELF'] = $REQUEST_URI;
    ...
    // set template file
    if (empty($template_file_in)) {
      // using template file set previously
      if (empty($this->template_contents)) {
        // CRITICAL ERROR!
        // no template file set
        // return with an error (FALSE)
        return FALSE;
      }
      else {
        $template_contents = $this->template_contents;
      }
    }
    else {
      // template file set explicitly calling parse method
      if (!$template_contents = $this->readTemplateFile($template_file)) {
        // CRITICAL ERROR!
    ...
    // redering begins.... NOW
    if (!$rendered_page = $this->doParse($template_contents, $data)) {
      // CRITICAL ERROR!
  ...
  **/
  function doParse ($block, $data) {
    $tags = array();
    $loop_count = 0;
    // procede with other template inclusions
    while (preg_match('/<TPL_INCLUDE/i', $block)
           && $loop_count < $this->max_include_levels) {
      preg_match_all ("/<(TPL_INCLUDE[^>]*):([^>]*)>/is",
                      $block, $tags, PREG_SET_ORDER);
      for ($i=0; $i < sizeof($tags); $i++) {
        if ($replace = $this->readTemplateFile($tags[$i][2])) {
          // include file read. place it into block
        }
        else {
          // error reading include file.
          // Do not stop parsing whole page.
          // Show an error in this place if warn_level is set to
          // TPL_WARN_CRITICAL or TPL_WARN_ALL
          $replace = $this->reportErrorMessage('Include file [' . 
                                               $tags[$i][2] . '] not found!');
        }
        $block = preg_replace("~".$tags[$i][0]."~", $replace, $block);
      }
      $loop_count++;
    }
    
    // gather all template tags
    preg_match_all ("/<([\/]?TPL[^>]*):([^>]*)>/is",
                    $block, $tags, PREG_SET_ORDER);
    // parse template
    $prefix = '';
    $offset = 0;
    for ($i=0; $i < sizeof($tags); $i++) {
      ...
      if ($type == '/TPL_SUB') {
        // end of the block
        if (preg_match('/'.$title.'.$/i', $prefix)) {
          $prefix = preg_replace('/'.$title.'.$/', '', $prefix);
        }
        else {
      ...
      if (!preg_match('/TPL_OBJECT/i', $type) && !preg_match('/TPL_INPUT/i', $type)) {
        $pos = strpos($block, $tag, $offset);
        $newtag = "<$type:$prefix$title>";
    ...
    if ($block = $this->populateBlock($block, $data)) {
      // everything's ok
	if ($this->debug) {
        }
    }
    else {
      // there was an error populating template with data
      // return with an error (FALSE)
      return FALSE;
    }
    
    // process form elements in template
    global $frm;
    if ($this->process_inputs) {
      if (is_object($frm)) {
        // process only if $frm is an instatiated object
        preg_match_all ("/<TPL_INPUT:([^.>]*)[.]?([^>]*)>/i",
                        $block, $tags, PREG_SET_ORDER);
        for ($i=0; $i < sizeof($tags); $i++) {
          $block = str_replace($tags[$i][0], 
                               $frm->getFieldWithValue(strtolower($tags[$i][1]),
                                                      strtolower($tags[$i][2])),
                               $block);
        }
      }
      else {
        // form object $frm has not been instantiated. issue warning
        //$block = $this->reportWarningMessage('Form object has not been '
    ...
    for ($i=0; $i < sizeof($tags); $i++) {
      ...
      switch ($type) {
        ...
        case '/TPL_OBJECT':
          ...
          if (!isset($object_result[$object_name][$object_instance])) {
            // check if cached version of this object exists
            if ($this->isObjectCached($object_name, $object_instance)
                && $object_result[$object_name][$object_instance] = 
                     $this->getCachedObject($object_name, $object_instance)) {
              // cached version was used
            }
            else {
              // instantiate the object only if there was no same object with the
              // same parameters specified before
              $object_result[$object_name][$object_instance] = 
                                            $this->procObject($object_name,
                                                              $object_params,
                                                              $object_output);
              // save object cache
              if ($this->cache_level === TPL_CACHE_ALL 
                  || $this->cache_level === TPL_CACHE_OBJECTS) {
                $this->saveCachedObject($object_result[$object_name]
                                                      [$object_instance],
                                        $object_name, 
                                        $object_instance);
              }
          ...
          $block = eregi_replace("<TPL_OBJECT_CLONE:$object_name>",
                                 $object_result[$object_name][$object_instance],
                                 $block);
    ...
    // report unused template tags warnings
    if (preg_match('(<[/]?TPL.*>)', $block)) {
      // remove unused template tags
      $block = preg_replace('/<[_]?TPL_SUB:([^>]*)>.*?<\/TPL_SUB:\1>/is', '', $block);
      $block = preg_replace('/<TPL:.*?>/', '', $block);
    }
    return $block;
  }
