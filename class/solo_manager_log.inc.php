...
class SoloManagerLog {
 
  function SoloManagerLog() {

  }

  function logEvent ($user_id, $event_type, $value, $season_id, $team1_id='', $team2_id='', $str_value=""){
    global $db;
...
//$db->showquery =  true;
    $sql_count = "SELECT COUNT(UL.ENTRY_ID) ROWS
                   FROM manager_log UL
                   WHERE EVENT_TYPE <> 3 AND SEASON_ID=".$season_id; 
    $db->query($sql_count);
    $count = 0;
    ...
    while ($row = $db->nextRow()) {
       ...
       $log_item['LOG_ENTRY'] = str_replace("%s", $row['STR_VALUE'], $log_item['LOG_ENTRY']);
       $log_item['UTC'] = $utc;
    ...
    $sql="SELECT DISTINCT UL.*, B.LAST_NAME, B.FIRST_NAME, T.TEAM_NAME2 as TEAM1, T2.TEAM_NAME2 as TEAM2, 
		T.TEAM_TYPE, CD.COUNTRY_NAME,
           ...
           FROM manager_log UL
                ...
                left join countries_details CD on CD.ID=T.COUNTRY and CD.lang_id=".$_SESSION['lang_id']."
                left join teams T2 on T2.TEAM_ID=UL.TEAM2_ID and UL.TEAM2_ID > 0
          WHERE EVENT_TYPE <> 3 AND ENTRY_ID=".$last_id." AND SEASON_ID=".$season_id;
    $db->query($sql);
    $c = 0;
    $log = "";
    if ($row = $db->nextRow()) {
