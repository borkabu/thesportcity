...
// include common header
  ...
  $manager_tournament_filter_box = $manager_tournamentbox->getManagerTournamentFilterBox($manager_tournament->getSeason());
  $rules = $pagebox->getPage(15);

  $smarty->assign("manager_tournament_filter_box", $manager_tournament_filter_box);
  $smarty->assign("rules", $rules);
  $start = getmicrotime();
  $content .= $smarty->fetch('smarty_tpl/f_manager_tournament.smarty');    
  $stop = getmicrotime();
  if (isset($_GET['debugphp']))
    echo 'smarty_tpl/f_manager_tournament.smarty'.($stop-$start);
