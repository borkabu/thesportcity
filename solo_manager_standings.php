...
//$tpl->setCacheTtl(60);
  ...
  $opt = array(
    ...
    $order = $param['order'];
    $so_fields = array('POINTS', 'DATE_CREATED', 'USER_NAME', 'PLACE');
    ...
    $sql= "SELECT DISTINCT C.ID, C.LATIN_NAME, C.ORIGINAL
		FROM users U, solo_manager_users MU, countries C
...
//print_r($country);
    ...
    $where_table=" solo_manager_standings MS ";
    $external_source = "";
    if (isset($_SESSION['external_user']) && !isset($_GET['all'])) {
      $where_table=" solo_manager_standings_external MS ";
    ...
    if (!empty($_GET['tour_id'])) {
      ...
      $sql="SELECT COUNT(*) ROWCOUNT
          FROM users U, solo_manager_users MU, solo_manager_standings MS
         WHERE MS.USER_ID = U.USER_ID 
	   AND MS.SEASON_ID=".$manager->mseason_id."
 	   AND MU.SEASON_ID=".$manager->mseason_id."
    ...
    if ($user_count > 0) {
      if (!empty($_GET['tour_id'])) {
        $sql = "SELECT U.USER_NAME, U.USER_ID, MUT.POINTS AS KOEFF, MUT.PLACE_TOUR AS PLACE, MU.SEASON_ID, ROUND(MUT.RATING, 2) as RATING, 
			MUT.USER_ID AS PLAYER_ID, MU.TEAM_NAME, MU.ALLOW_VIEW, 
...
//echo $sql;
        ...
        $sql="SELECT U.USER_NAME, U.USER_ID, MU.ALLOW_VIEW, MU.TEAM_NAME, 
  		   ...
  		   C.CCTLD, CD.COUNTRY_NAME, MU.CHALLENGES_WON, MU.CHALLENGES_LOST, MU.CHALLENGES_WON + MU.CHALLENGES_LOST CHALLENGES_SUM,
		   MU.BATTLES_WON, MU.BATTLES_LOST, MU.BATTLES_WON + MU.BATTLES_LOST BATTLES_SUM, MU.SOURCE
	          FROM users U, solo_manager_users MU, ".$where_table.", countries C
			LEFT JOIN countries_details CD ON C.ID=CD.ID aND CD.LANG_ID=".$_SESSION['lang_id']."
                   WHERE MS.USER_ID = U.USER_ID 
	 	    AND MS.SEASON_ID=".$manager->mseason_id."
 	  	    AND MU.SEASON_ID=".$manager->mseason_id."
      ...
      $db->query($sql);
        ...
        $sql="SELECT COUNT(*) ROWCOUNT
          FROM users U, solo_manager_users MU
...
//echo $sql;
  ...
  $start = getmicrotime();
  $content .= $smarty->fetch('smarty_tpl/solo_manager_standings.smarty');    
  $stop = getmicrotime();
  if (isset($_GET['debugphp']))
    echo 'smarty_tpl/solo_manager_standings.smarty'.($stop-$start);
