...
//$db->showquery=true;
  ...
  $pleague = new League("solo_manager", $_GET['league_id']);
    ...
    if ($code != "" && $code == $_POST['league_code']) {
     // join
       ...
       if ($pleague->league_info['ENTRY_FEE'] == 0) {
         $db->delete('solo_manager_leagues_members', "LEAGUE_ID=".$_GET['league_id']." AND USER_ID=".$auth->getUserId());
         $db->insert('solo_manager_leagues_members', $sdata);
         unset($udata);

         $udata['STATUS'] = "1";  
         $udata['JOINED'] = "JOINED+1";  
         $db->update('solo_manager_leagues', $udata, ' LEAGUE_ID='.$_GET['league_id']);
       ...
       $sql="SELECT COUNT(USER_ID) USERS from solo_manager_leagues_members WHERE STATUS in (1,2) and league_id =". $_GET['league_id'];
       $db->query($sql);     
       if ($row = $db->nextRow()) {
         if ($row['USERS'] >= $pleague->league_info['PARTICIPANTS'] && $pleague->league_info['PARTICIPANTS'] > 0) {
           ...
           $db->update("solo_manager_leagues", $sdata, "LEAGUE_ID=".$_GET['league_id']);   
         }
       }
    } else {
      // cannot verify code
  ...
  if (isset($_POST['post_comment']) && $auth->userOn() &&
     ...
     $forumbox->addPost($forums['SOLO_MANAGER_LEAGUES'], $_POST['topic_id'], $_POST['item_id']);
  }
       ...
       $sql= "SELECT ML.*, MLM.*, U.USER_NAME, MSS.POINTS, MSS.PLACE, 
		C.CCTLD, CD.COUNTRY_NAME, MU.ALLOW_VIEW
	       ...
	       manager_seasons MS, users U
                 LEFT JOIN solo_manager_standings MSS ON MSS.USER_ID=U.USER_ID AND MSS.SEASON_ID=".$manager->mseason_id."
		 LEFT JOIN solo_manager_users MU ON MU.USER_ID=U.USER_ID AND MU.SEASON_ID=".$manager->mseason_id."
	      , countries C
	...
	if (($auth->hasSupporter() && !$manager->manager_trade_allow) || $row['ALLOW_VIEW'] == '1')
      ...
      if ($auth->userOn()) { 
        $sql= "SELECT DISTINCT MLV.VOTE, MLM.LEAGUE_ID, MLM.USER_ID
            FROM solo_manager_leagues_members MLM
			left join solo_manager_leagues_votes MLV
...
//      $pleague = new League("solo_manager", $_GET['league_id']);
	...
	if (isset($_POST['refund_league'])) {
          ...
          foreach( $elmembers as $member) {
            if (($member['STATUS'] == 2 || $member['STATUS'] == 4) && $member['REFUNDED'] == 0) {
              ...
              $sdata['REFUNDED'] = 1;
              $db->update('solo_manager_leagues_members', $sdata, "LEAGUE_ID=".$_GET['league_id']." AND USER_ID=".$member['USER_ID']);
            }
          }
	  $db->update('solo_manager_leagues', $sdata, "LEAGUE_ID=".$_GET['league_id']);
          unset($sdata);
	 ...
	 FROM solo_manager_users_tours MUT
    ...
    if (!isset($_GET['league_id'])) {
      ...
      if ((isset($_GET['all']) && $_GET['all'] == 'y') || !$auth->userOn()) {
        $leagues = $manager->getLeagues($param['where'], "solo_");
        $leagues_number = $manager->leagues;
        $filtering['ALL_LEAGUES'] = 1;
      }
      else {
        $leagues = $manager_user->getLeagues($param['where'], "solo_");
    ...
    if ($auth->userOn() && !$manager_user->hasSoloLeague())
...
//print_r($league);
  ...
  $start = getmicrotime();
  $content .= $smarty->fetch('smarty_tpl/solo_manager_league.smarty');    
  $stop = getmicrotime();
  if (isset($_GET['debugphp']))
    echo 'smarty_tpl/solo_manager_league.smarty'.($stop-$start);

//$db->showquery=true;
  if (isset($_GET['league_id']) && $_GET['league_id'] != '') {
    $page_size = 40;
    $content .= $forumbox->getComments($_GET['league_id'], 'SOLO_MANAGER_LEAGUES', isset($_GET['page']) ? $_GET['page'] : 1);
  }
