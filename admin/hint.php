...
if (isset($_GET['del']) && !$ro && strcmp($_SESSION["_admin"][MENU_FORUM], 'FA') == 0) {
  $db->delete('hints_details', 'HINT_ID='.$_GET['del']);
  $db->delete('hints', 'HINT_ID='.$_GET['del']);
  header('Location: hint.php');
}
// activate
if (isset($_GET['activate']) && !$ro) {
  $db->update('hints', array('PUBLISH' => "'Y'"),'HINT_ID='.$_GET['activate']);
}
// deactivate
if (isset($_GET['deactivate']) && !$ro) {
  $db->update('hints', array('PUBLISH' => "'N'"),'HINT_ID='.$_GET['deactivate']);
}
// --- END DELETE -------------------------------------------------------------
...
if (!isset($perpage))
  $perpage = $page_size;
if (empty($order))
  $order = 'HINT_TYPE, HINT_TITLE asc';

// sorting
$param['order'] = $order;
$data['ORDER'] = $param['order'];
$so_fields = array('HINT_TITLE', 'PUBLISH');
...
$sql = "SELECT C.HINT_ID, CD.HINT_TITLE, CD.DESCR, C.PUBLISH, C.HINT_TYPE,
	       GROUP_CONCAT(CD2.LANG_ID) as LANGUAGES
        FROM hints  C 
		left JOIN hints_details CD ON C.HINT_ID = CD.HINT_ID  AND CD.LANG_ID=".$_SESSION['lang_id']."
		left join hints_details CD2 ON CD2.HINT_ID=C.HINT_ID
        WHERE ".$param['where']."
	GROUP BY C.HINT_ID
...
while ($row = $db->nextRow()) {
  $data['ITEM'][$c] = $row;
  $data['ITEM'][$c]['HINT_TYPE'] = $hint_types[$row['HINT_TYPE']];

  $used_langs = explode(",", $row['LANGUAGES']);
  foreach ($languages as $language) {
    if (in_array($language['ID'], $used_langs)) {
      $data['ITEM'][$c]['LANGS'][$language['ID']]['USED'][0] = $language;
      $data['ITEM'][$c]['LANGS'][$language['ID']]['USED'][0]['HINT_ID'] = $row['HINT_ID'];
    }
    else {
      $data['ITEM'][$c]['LANGS'][$language['ID']]['NOTUSED'][0] = $language;
      $data['ITEM'][$c]['LANGS'][$language['ID']]['NOTUSED'][0]['HINT_ID'] = $row['HINT_ID'];
    }
  }

  if (strcmp($_SESSION["_admin"][MENU_FORUM], 'FA') == 0)
    $data['ITEM'][$c]['DEL'][0]['DEL_URL'] = $_SERVER['PHP_SELF'].url('del', $row['HINT_ID']);
  ...
  if ($row['PUBLISH'] == 'Y')
    $data['ITEM'][$c]['ACT_URL'] = $_SERVER['PHP_SELF'].url('deactivate', $row['HINT_ID']);
  else
    $data['ITEM'][$c]['ACT_URL'] = $_SERVER['PHP_SELF'].url('activate', $row['HINT_ID']);
...
$tpl->setTemplateFile('../tpl/adm/hint.tpl.html');
$tpl->addData($data);
echo $tpl->parse();
