...
if(isset($_POST['form_save'])&&!$ro){
  ...
  if (!$error) {
    ...
    if (empty($_GET['id'])) {
        $db->insert('bracket_subseasons', $sdata);
    } else {
        $db->update('bracket_subseasons', $sdata, " ID=".$_GET['id']);       
    }
...
// new or edit?
if (isset($_GET['id'])) {
  $db->select('bracket_subseasons', '*', "ID=".$_GET['id']);
  if (!$row = $db->nextRow()) {
...
$tpl->setTemplateFile('../tpl/adm/bracket_subseasons_edit.tpl.html');
$tpl->addData($data);
echo $tpl->parse();
