...
if (isset($_GET['del']) && !$ro && strcmp($_SESSION["_admin"][MENU_ACTIONS_MANAGER], 'FA') == 0) {
  $db->delete('manager_tournament', 'MT_ID='.$_GET['del']);
}
// activate
if (isset($_GET['activate']) && !$ro) {
  $db->update('manager_tournament', array('PUBLISH' => "'Y'"),'MT_ID='.$_GET['activate']);
}
// deactivate
if (isset($_GET['deactivate']) && !$ro) {
  $db->update('manager_tournament', array('PUBLISH' => "'N'"),'MT_ID='.$_GET['deactivate']);
}
// --- END UPDATE -------------------------------------------------------------
...
while ($row = $db->nextRow()) {
  ...
  $sql = "SELECT F.MT_ID, F.SEASON_ID, FD.SEASON_TITLE, F.PUBLISH, 
                SUBSTRING(F.START_DATE, 1, 10) START_DATE,
                SUBSTRING(F.END_DATE, 1, 10) END_DATE,
		REGISTRATION_END_DATE,
 	        GROUP_CONCAT(FD2.LANG_ID) as LANGUAGES
	  FROM manager_tournament F 
		left JOIN manager_tournament_details FD ON F.MT_ID = FD.MT_ID AND FD.LANG_ID=".$_SESSION['lang_id']."
		left join manager_tournament_details FD2 ON FD2.MT_ID =F.MT_ID 
...
while ($row = $db->nextRow()) {
  ...
  foreach ($languages as $language) {
    if (in_array($language['ID'], $used_langs)) {
      ...
      $data['ITEM'][$c]['LANGS'][$language['ID']]['USED'][0]['MT_ID'] = $row['MT_ID'];
    }
    else {
      $data['ITEM'][$c]['LANGS'][$language['ID']]['NOTUSED'][0] = $language;
      $data['ITEM'][$c]['LANGS'][$language['ID']]['NOTUSED'][0]['MT_ID'] = $row['MT_ID'];
    }
  }

  if (strcmp($_SESSION["_admin"][MENU_ACTIONS_MANAGER], 'FA') == 0)
    $data['ITEM'][$c]['DEL'][0]['DEL_URL'] = $_SERVER['PHP_SELF'].url('del', $row['MT_ID']);
  
  if ($row['PUBLISH'] == 'Y')
    $data['ITEM'][$c]['ACT_URL'] = $_SERVER['PHP_SELF'].url('deactivate', $row['MT_ID']);
  else
    $data['ITEM'][$c]['ACT_URL'] = $_SERVER['PHP_SELF'].url('activate', $row['MT_ID']);

  $data['ITEM'][$c]['EDIT_URL'] = 'manager_tournament_season_edit.php'; 
  if ($c & 2 > 0)
...
$tpl->setTemplateFile('../tpl/adm/manager_tournament.tpl.html');
$tpl->addData($data);
echo $tpl->parse();
