...
if(isset($_POST['form_save'])&&!$ro){
  ...
  if (!$error) {
    ...
    if(!empty($_GET["cat_id"]) && !empty($_POST["lang_id"])){
      ...
      $db->replace('cats_details', $sdata);
    }
    else {
      // INSERT
	$tdata['PUBLISH'] = "'N'";
	$db->insert('cats', $tdata);
	$sdata['cat_id'] = $db->id();
...
if (isset($_GET['cat_id'])) {
  ...
  $db->select('cats_details',$fields,"CAT_ID=".$_GET['cat_id']." AND LANG_ID=".$_SESSION['lang_id']);
...
$tpl->setTemplateFile('../tpl/adm/cat_edit.tpl.html');
$tpl->addData($data);
echo $tpl->parse();
