...
if(isset($_POST['form_save'])&&!$ro){
  // required fields
  $s_fields = '';
  $i_fields = array('season_id', 'mseason_id');
  $d_fields = '';
  $c_fields = '';
  $r_fields = array('season_id', 'mseason_id');
  ...
  if (!$error) {
    ...
    if (empty($_GET['id'])) {
        $db->insert('manager_subseasons', $sdata);
    } else {
        $db->update('manager_subseasons', $sdata, " ID=".$_GET['id']);       
    }
...
// new or edit?
if (isset($_GET['id'])) {
  $db->select('manager_subseasons', '*', "ID=".$_GET['id']);
  if (!$row = $db->nextRow()) {
...
$data['MSEASON_ID'] = $_GET['mseason_id'];

// content
$tpl->setCacheLevel(TPL_CACHE_NOTHING);
$tpl->setTemplateFile('../tpl/adm/manager_subseasons_edit.tpl.html');
$tpl->addData($data);
echo $tpl->parse();
