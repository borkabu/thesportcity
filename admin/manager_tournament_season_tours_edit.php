...
if(isset($_POST['form_save'])&&!$ro){
  // required fields
  $s_fields = '';
  $i_fields = array('mt_id', 'number', 'round');
  ...
  if (!$error) {
    ...
    $sdata = buildSaveData($s_fields, 
     ...
     if (!isset($_GET['tour_id'])) {
      ...
      {
        $db->update('manager_tournament_tours', $sdata, " TOUR_ID=".$_GET['tour_id']);       
      }
...
// new or edit?
if (isset($_GET['tour_id'])) {
  $db->select('manager_tournament_tours', '*', "TOUR_ID=".$_GET['tour_id']);
  if (!$row = $db->nextRow()) {
    // ERROR! No such record. redirect to list
    header('Location: manager_tournament.php');
    exit;
   }
  else {
...
$tpl->setTemplateFile('../tpl/adm/manager_tournament_tours_edit.tpl.html');
$tpl->addData($data);
echo $tpl->parse();
