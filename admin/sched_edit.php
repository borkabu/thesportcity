...
if(isset($_POST['form_save']) && !$ro){
  ...
  // required fields
  $r_fields = array('team_id1', 'team_id2', 
...
if (isset($_GET['game_id'])) {
  // edit
  $fields = 'GAME_ID, SEASON_ID, TEAM_ID1, TEAM_ID2, 
            SUBSTRING(START_DATE, 1, 16) START_DATE,
            PUBLISH';
  $db->select('games', $fields, "GAME_ID=".$_GET['game_id']);
  if (!$row = $db->nextRow()) {
...
$data['TEAM_ID1'] = inputTeams('team_id1');
$data['TEAM_ID2'] = inputTeams('team_id2');

// content
$tpl->setCacheLevel(TPL_CACHE_NOTHING);
$tpl->setTemplateFile('../tpl/adm/sched_edit.tpl.html');
$tpl->addData($data);
echo $tpl->parse();
