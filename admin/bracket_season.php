...
if (isset($_GET['del']) && !$ro && strcmp($_SESSION["_admin"][MENU_ACTIONS_WAGER], 'FA') == 0) {
  $db->delete('bracket_seasons', 'SEASON_ID='.$_GET['del']);
  $db->delete('bracket_seasons_details', 'SEASON_ID='.$_GET['del']);
}

// activate
if (isset($_GET['activate']) && !$ro) {
  $db->update('bracket_seasons', array('PUBLISH' => "'Y'"),'SEASON_ID='.$_GET['activate']);
}
// deactivate
if (isset($_GET['deactivate']) && !$ro) {
  $db->update('bracket_seasons', array('PUBLISH' => "'N'"),'SEASON_ID='.$_GET['deactivate']);
}


if (isset($_GET['copy_season']) && !$ro) {
  $sql = "INSERT INTO bracket_seasons (START_DATE, END_DATE, 
	 		PRIZE_FUND, NEWSLETTER_ID) 
		select START_DATE, END_DATE, PRIZE_FUND, NEWSLETTER_ID
		from bracket_seasons where season_id=".$_GET['season_id'];
  $db->query($sql);
  $new_season_id = $db->id();

  $sql = "INSERT INTO bracket_seasons_details (season_id, tseason_title, prizes, lang_id)  
		select ".$new_season_id.", concat('copy of ', tseason_title), prizes, lang_ID 
		from bracket_seasons_details
...
$sql = "SELECT TS.SEASON_ID, TSD.TSEASON_TITLE, TS.PUBLISH,               
		...
		left join bracket_seasons_details TSD2 ON TSD2.SEASON_ID=TS.SEASON_ID
...
$tpl->setTemplateFile('../tpl/adm/bracket_season.tpl.html');
$tpl->addData($data);
echo $tpl->parse();
