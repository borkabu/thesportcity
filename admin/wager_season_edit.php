...
if (isset($_GET['del2']) && !$ro && strcmp($_SESSION["_admin"][MENU_ACTIONS_WAGER], 'FA') == 0) {
  $db->delete('wager_subseasons', 'ID='.$_GET['del2']);
}
...
if(isset($_POST['form_save']) && !$ro){
  // required fields
  $s_fields = '';
  $i_fields = array ('money', 'prize_fund', 'newsletter_id');
  ...
  if (!$error) {
    ...
    if (!empty($_GET['season_id'])) {
      // UPDATE
      $db->update('wager_seasons', $sdata, "SEASON_ID=".$_GET['season_id']);
      $tdata['season_id'] = $_GET['season_id'];
      $db->select('wager_seasons_details', "*", "SEASON_ID=".$_GET["season_id"]." AND LANG_ID=".$_POST['lang_id']);
      if ($row = $db->nextRow())
	  $db->update('wager_seasons_details', $tdata, "SEASON_ID=".$_GET["season_id"]." AND LANG_ID=".$_POST['lang_id']);
      else $db->insert('wager_seasons_details', $tdata);
    }
    else {
      // INSERT
      $db->insert('wager_seasons', $sdata);
      $tdata['season_id'] = $db->id();
      $db->insert('wager_seasons_details',$tdata);
    }
...
if (isset($_GET['season_id'])) {
  // edit
  $sql = "SELECT TS.SEASON_ID, TSD.TSEASON_TITLE, TSD.PRIZES, TS.PUBLISH, TS.MONEY,              
           TS.NEWSLETTER_ID, TS.PRIZE_FUND,
  ...
  $data['NEWSLETTER_ID'] = inputWagerNewsletters('newsletter_id', isset($PRESET_VARS['newsletter_id']) ? $PRESET_VARS['newsletter_id'] : '');
  // subseasons
  $sql = "SELECT FD.SEASON_TITLE, WSS.*
        FROM wager_subseasons WSS, wager_seasons WS, manager_seasons MS
		LEFT JOIN manager_seasons_details FD ON FD.SEASON_ID=MS.SEASON_ID AND FD.LANG_ID=".$_SESSION['lang_id']."
...
echo $sql;
 ...
 while ( $row = $db->nextRow () ) {
	...
	if (strcmp($_SESSION["_admin"][MENU_ACTIONS_WAGER], 'FA' ) == 0)
...
$tpl->setTemplateFile('../tpl/adm/wager_season_edit.tpl.html');
$tpl->addData($data);
echo $tpl->parse();
