  ...
  if (!$error) {
    ...
    $sdata = buildSaveData($s_fields, 
     ...
     if (!isset($_GET['tour_id'])) {
      ...
      {
        $db->update('bracket_tours', $sdata, " TOUR_ID=".$_GET['tour_id']);       
      }
...
// new or edit?
if (isset($_GET['tour_id'])) {
  $db->select('bracket_tours', '*', "TOUR_ID=".$_GET['tour_id']);
  if (!$row = $db->nextRow()) {
    // ERROR! No such record. redirect to list
    header('Location: bracket_seasons.php');
    exit;
   }
  else {
...
$tpl->setTemplateFile('../tpl/adm/bracket_season_tours_edit.tpl.html');
$tpl->addData($data);
echo $tpl->parse();
