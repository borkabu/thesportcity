...
$content = '';
  ...
  if (isset($_GET['player_id']) && isset($_GET['league_id'])) {
     $sql = "SELECT MM.CURRENT_VALUE_MONEY, MM.SEASON_ID, RML.TITLE, RML.SEASON_ID, RML.FREE_TRANSFER_FEE
...
//       $where_played = "AND MM.PLAYED > 0" ;
     ...
     $sql= "SELECT MM.LAST_NAME, MM.FIRST_NAME, MM.USER_ID, MM.POSITION_ID1, 
			MM.CURRENT_VALUE_MONEY, MM.PLAYED, MM.PLAYER_STATE,
	...
	LIMIT 7";
     ...
     $smarty->assign("old_player_id", $_GET['player_id']);
     $smarty->assign("league_name", $title);
     $smarty->assign("selling_price", $selling_price);
     $smarty->assign("players", $players);
     $smarty->assign("free_transfer", true);
     $smarty->assign("free_transfer_fee", $free_transfer_fee);
     if ($free_transfer_fee > $auth->getCredits())
       $smarty->assign("not_enough_credits", true);
  }                                      
